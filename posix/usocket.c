...
/* Re-applied from 137ec58f after the upstream rewrite (upstream still ships 64 kB).
 * The default is one page, so a large X11 transfer otherwise crosses the channel in
 * hundreds of block/wake cycles. A server must set SO_RCVBUF on the LISTENING fd
 * before accept(), since an accepted socket's rx ring is sized from the listener's.
 * NB: the old "+12%" figure predates upstream's internally-looping uchannel_write()
 * and should be re-measured before it is quoted again. */
