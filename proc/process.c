...
static void process_destroy(process_t *p)
{
	...
	/* ours: invalidate the magic so a later use of a freed process_t is
	 * recognisable rather than plausible (see vm/map.c's corrupt-pointer path) */
	p->magic = 0U;
...
static void process_exec(thread_t *current, process_spawn_t *spawn)
{
	...
	if (err != EOK) {
		 ...
		 * The termination itself is upstream's since 2026-09-17 -- a SIGKILL to the
		 * process rather than setting ->exit directly, now that signal delivery lives
		 * in the kernel. */
